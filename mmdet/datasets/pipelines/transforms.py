...
class Resize(object):
    ...
    def __init__(self,
        ...
        self.fix_empty_masks = fix_empty_masks
    ...
    def _safe_resize_single_mask(self, mask, scale_factor):
        scaled_mask = mmcv.imrescale(
            mask, scale_factor, interpolation='nearest')
        if not self.fix_empty_masks:
            return scaled_mask
        if not np.any(scaled_mask):
            coordinates = np.nonzero(mask)
            center = tuple(map(lambda x : int(np.round(np.mean(x) * scale_factor)), coordinates))
            scaled_mask[center[0], center[1]] = True
        return scaled_mask
    ...
    def _resize_masks(self, results):
        for key in results.get('mask_fields', []):
            if results[key] is None:
                continue
            if self.fix_empty_masks:
                assert len(results[key]) == results['gt_bboxes'].shape[0]
                assert len(results[key]) == results['gt_labels'].shape[0]
                for mask in results[key]:
                    assert np.any(mask)
            if self.keep_ratio:
                masks = [
                    self._safe_resize_single_mask(mask, results['scale_factor'])
                    for mask in results[key]
                ]
            else:
            ...
            if self.fix_empty_masks:
                ...
                for i in sorted(to_remove, reverse=True):
                    print(str(results['gt_bboxes'][i]) + ' ' + str(results['gt_labels'][i]))
                    results['gt_bboxes'] = np.delete(results['gt_bboxes'], i, axis=0)
                    results['gt_labels'] = np.delete(results['gt_labels'], i, axis=0)
                    del masks[i]
                for mask in masks:
                    assert np.any(mask)
                assert len(masks) == results['gt_bboxes'].shape[0]
                assert len(masks) == results['gt_labels'].shape[0]
            results[key] = masks
