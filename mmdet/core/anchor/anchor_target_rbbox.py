...
def anchor_target_rbbox_single(flat_anchors,
    ...
    bbox_targets =  torch.zeros(num_valid_anchors, 6 if modulated_loss else 5).to(anchors.device)
    ...
    # implementation B
    gt_obbs = gt_mask_bp_obbs(gt_masks, with_module, mod_90_deg)
    ...
    if len(pos_inds) > 0:
        ...
        #     raise Exception
        pos_ext_bboxes = hbb2obb_v2(sampling_result.pos_bboxes, default_angle=default_angle)
        if modulated_loss:
            pos_bbox_targets = dbbox2delta_modulated_loss(pos_ext_bboxes,
                                              pos_gt_obbs_ts,
                                              target_means, target_stds)
        elif with_module:
