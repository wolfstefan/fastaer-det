...
def dbbox2delta_modulated_loss(proposals, gt, means = [0, 0, 0, 0, 0], stds=[1, 1, 1, 1, 1]):
    """
    This version removes the module operation
    :param proposals: (x_ctr, y_ctr, w, h, angle)
            shape (n, 5)
    ...
    :param means:
    :param stds:
    :return: encoded targets: shape (n, 6)
    """
    proposals = proposals.float()
    gt = gt.float()
    gt_widths = gt[..., 2]
    ...
    dh = torch.log(gt_heights / proposals_heights)
    dangle = gt_angle - proposals_angle
    #ratio = gt_widths / gt_heights
    ratio = proposals_widths / proposals_heights
    deltas = torch.stack((dx, dy, dw, dh, dangle), -1)

    means = deltas.new_tensor(means).unsqueeze(0)
    stds = deltas.new_tensor(stds).unsqueeze(0)
    deltas = deltas.sub_(means).div_(stds)
    deltas = torch.cat((deltas, ratio.unsqueeze(-1)), dim=1)
...
def delta2dbbox_v3(Rrois,
    ...
    means = deltas.new_tensor(means).view(1, -1).repeat(1, num_preds // 5)
    stds = deltas.new_tensor(stds).view(1, -1).repeat(1, num_preds // 5)
    denorm_deltas = deltas * stds + means
    ...
    dw = dw.clamp(min=-max_ratio, max=max_ratio)
    dh = dh.clamp(min=-max_ratio, max=max_ratio)
    Rroi_x = (Rrois[:, 0]).unsqueeze(1)
    Rroi_y = (Rrois[:, 1]).unsqueeze(1)
    Rroi_w = (Rrois[:, 2]).unsqueeze(1)
    Rroi_h = (Rrois[:, 3]).unsqueeze(1)
    Rroi_angle = (Rrois[:, 4]).unsqueeze(1)
    # import pdb
    # pdb.set_trace()
    gx = dx * Rroi_w * torch.cos(Rroi_angle) \
         - dy * Rroi_h * torch.sin(Rroi_angle) + Rroi_x
    ...
    bboxes = torch.stack([gx, gy, gw, gh, gangle], dim=2).view(list(map(int, deltas.shape)))
    return bboxes
...
def gt_mask_bp_obbs(gt_masks, with_module=True, mod_90_deg=False):

    # trans gt_masks to gt_obbs
    gt_polys = mask2poly(gt_masks)
    gt_bp_polys = get_best_begin_point(gt_polys)
    gt_obbs = polygonToRotRectangle_batch(gt_bp_polys, with_module, mod_90_deg)
...
def hbb2obb_v2(boxes, default_angle=-np.pi / 2):
    """
    fix a bug
    ...
    initial_angles = c_bboxes.new_ones((num_boxes, 1)) * default_angle
    # initial_angles = -torch.ones((num_boxes, 1)) * np.pi/2
    dbboxes = torch.cat((c_bboxes, initial_angles), 1)
...
def polygonToRotRectangle_batch(bbox, with_module=True, mod_90_deg=False):
    """
    ...
    if mod_90_deg:
        ...
        angle %= np.pi
        indices = angle > np.pi / 2
        angle[indices] -= np.pi / 2
        temp = w[indices]
        w[indices] = h[indices]
        h[indices] = temp
    elif with_module:
