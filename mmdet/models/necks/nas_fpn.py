...
class MergingCell(nn.Module):
    ...
    def _resize(self, x, size):
        if x.shape[-2:] == size:
            ...
            kernel_size = int(x.shape[-1] // size[-1])
...
class NASFPN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for stage in self.fpn_stages:
            ...
            p4_h = int(p4_size[0])
            p4_w = int(p4_size[1])
            p4_1 = stage['gp_64_4'](p6, p4, out_size=(p4_h, p4_w))
            # sum(p4_1, p4) -> p4_2
            p4_2 = stage['sum_44_4'](p4_1, p4, out_size=(p4_h, p4_w))
            # sum(p4_2, p3) -> p3_out
            p3_size = p3.shape[-2:]
            p3_h = int(p3_size[0])
            p3_w = int(p3_size[1])
            p3 = stage['sum_43_3'](p4_2, p3, out_size=(p3_h, p3_w))
            # sum(p3_out, p4_2) -> p4_out
            p4 = stage['sum_34_4'](p3, p4_2, out_size=(p4_h, p4_w))
            # sum(p5, gp(p4_out, p3_out)) -> p5_out
            p5_size = p5.shape[-2:]
            p5_h = int(p5_size[0])
            p5_w = int(p5_size[1])
            p5_tmp = stage['gp_43_5'](p4, p3, out_size=(p5_h, p5_w))
            p5 = stage['sum_55_5'](p5, p5_tmp, out_size=(p5_h, p5_w))
            # sum(p7, gp(p5_out, p4_2)) -> p7_out
            p7_size = p7.shape[-2:]
            p7_h = int(p7_size[0])
            p7_w = int(p7_size[1])
            p7_tmp = stage['gp_54_7'](p5, p4_2, out_size=(p7_h, p7_w))
            p7 = stage['sum_77_7'](p7, p7_tmp, out_size=(p7_h, p7_w))
            # gp(p7_out, p5_out) -> p6_out
            p6_size = p6.shape[-2:]
            p6_h = int(p6_size[0])
            p6_w = int(p6_size[1])
            p6 = stage['gp_75_6'](p7, p5, out_size=(p6_h, p6_w))
