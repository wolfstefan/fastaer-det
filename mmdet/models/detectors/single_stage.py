...
class SingleStageDetector(BaseDetector):
    ...
    def simple_test(self, img, img_meta, rescale=False, postprocess=True):
        ...
        if postprocess:
            bbox_results = [
                self.postprocess(det_bboxes, det_labels, img_meta, rescale=rescale)
                for det_bboxes, det_labels in bbox_list
            ]
            return bbox_results[0]
            #return self.postporcess(bbox_list,  img_meta, rescale=rescale)
        else:
            return bbox_list[0][0], bbox_list[0][1]

    def postprocess(self, det_bboxes, det_labels, img_meta, rescale=False):
