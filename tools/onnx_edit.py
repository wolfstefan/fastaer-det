...
# Original repository: https://github.com/saurabh-shandilya/onnx-utils
# -----------------------------------------------
...
def traceDependentNodes(graph,name,node_input_names,node_map, initializer_map, global_inputs):
            ...
            if (noutput == name) and (n.name not in node_input_names):
                ...
                if n.name in node_map.keys():
                    for ninput in node_map[n.name].input:
                        if ninput in global_inputs:
                            continue
                        # trace input node's inputs 
                        node_input_names = traceDependentNodes(graph,ninput,node_input_names,node_map, initializer_map, global_inputs)
...
def onnx_edit(input_model, output_model, new_input_node_names, input_shape_type, input_shape_map, new_output_node_names, output_shape_type, output_shape_map, verify):
    ...
    Arguments: 
        ...
        input_type_map: dictionary/map of input node names to corresponding types.
        ...
        output_type_map: dictionary/map of output node names to corresponding type.
    ...
    for name in new_names:
        ...
        if(name in input_type_map.keys()):
            node_type = input_type_map[name]
        else:
            node_type = TensorProto.FLOAT
        if(name in input_shape_map.keys()):
            new_nv = helper.make_tensor_value_info(name, node_type, input_shape_map[name])
        else:
            new_nv = helper.make_tensor_value_info(name, node_type, None)    
    ...
    for name in new_names:
        if(name in output_type_map.keys()):
            node_type = output_type_map[name]
        else:
            node_type = TensorProto.FLOAT
        if(name in output_shape_map.keys()):
            new_nv = helper.make_tensor_value_info(name, node_type, output_shape_map[name])
        else:
            new_nv = helper.make_tensor_value_info(name, node_type, None)
    ...
    for new_output_node_name in new_output_node_names:
        valid_node_names=traceDependentNodes(graph,new_output_node_name,valid_node_names,node_map, initializer_map, new_input_node_names)
...
def parse_nodename_type_and_shape(name):
...
if __name__ == "__main__":
    ...
    if args.inputs:
        new_input_node_names, input_type_map, input_shape_map = parse_nodename_type_and_shape(args.inputs)
    else: 
    ...
    if args.outputs:
        new_output_node_names, output_type_map, output_shape_map = parse_nodename_type_and_shape(args.outputs)
    else:
        new_output_node_names = []
        output_type_map = {}
        output_shape_map = {}
        
    onnx_edit(args.input,args.output,new_input_node_names, input_type_map, input_shape_map, new_output_node_names, output_type_map, output_shape_map, not args.skipverify)
